...
const Auth = ({ defaultView = 'login' }: AuthProps) => {
  ...
  const handleOAuth = async (provider: 'google' | 'apple') => {
    ...
    try {
      ...
      if (error) throw error;
      // On success the browser is redirected to the provider, so no further action needed here.
    } catch (error: any) {
      toast({
        title: "Error",
        description: error.message || "No se pudo iniciar sesión. Por favor intenta nuevamente.",
        variant: "destructive",
      });
...
{/*
            ...
            >
              <svg className="h-5 w-5" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                <path d="M16.365 1.43c0 1.14-.42 2.21-1.13 3.01-.85.96-2.27 1.71-3.45 1.62-.14-1.11.43-2.27 1.1-3.01.78-.86 2.18-1.55 3.34-1.62.05.33.14.66.14 1zM20.5 17.04c-.55 1.27-.82 1.83-1.53 2.95-.99 1.56-2.39 3.5-4.12 3.51-1.54.01-1.93-1-4.02-.99-2.09.01-2.52.99-4.06.98-1.73-.01-3.06-1.76-4.05-3.32C-.06 16.61-.36 11.9 1.47 9.39c1-1.39 2.58-2.27 4.07-2.27 1.52 0 2.47.99 3.73.99 1.22 0 1.96-.99 3.72-.99 1.34 0 2.76.73 3.78 2-3.32 1.82-2.78 6.55.46 7.92z" />
              </svg>
